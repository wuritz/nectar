...
public class IntComponent extends SettingComponent<Integer> {
    ...
    public IntComponent(int x, int y, int width, int height, IntSetting setting) {
        ...
        Slider slider = new Slider(x+(width/2), y, width/2-35, height, intSetting.getMin(), intSetting.getMax(), this);
        components.add(slider);
    }

    @Override
    public void render(DrawContext context, int mouseX, int mouseY) {
        context.drawTextWithShadow(mc.textRenderer, intSetting.getName(), x, y, Color.WHITE.getPacked());
    ...
    public boolean updateValue(Integer newValue) {
        return intSetting.set(newValue);
    }
